...
class Turtle:
    ...
    def fillcolor(self, color):
        self._fillcolor = self._getColHex(color)
    ...
    def end_fill(self):
        if self._fillpath != None and len(self._fillpath) > 2:
            ...
            for i in range(1, num):
                if i+1 < num:
                    self._fb.triangle(round(x0), round(y0), round(self._fillpath[i][0]), round(self._fillpath[i][1]), round(self._fillpath[i+1][0]), round(self._fillpath[i+1][1]), self._fillcolor, 1)
        else:
